    ...
    <div class="card mb-4">
        <form action="{{ route('transaction.outgoing.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body row">
                        ...
                        <select class="form-select" id="status" name="status">
                            @foreach ($statuses as $status)
                                <option value="{{ $status->code }}"
                                    @if ($status->code == 'B') selected="selected" @endif
                ...
                <div class="col-sm-12 col-12 col-md-6 col-lg-4">
                    <x-input-form name="number" id="number" label="{{ __('model.letter.number') }}"
                        value="{{ $num2600 + 1 }}" />
                ...
                <div class="col-sm-12 col-12 col-md-6 col-lg-4">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="satker" name="satker" data-num2600="{{ $num2600 }}"
                            data-num2610="{{ $num2610 }}">
                            @foreach ($satkers as $satker)
                                <option value="{{ $satker->kode }}" @selected(old('sarker_kode') == $satker->kode)>
                                    {{ $satker->kode }}
                                </option>
                            @endforeach
                        </select>
